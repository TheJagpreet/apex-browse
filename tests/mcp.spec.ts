...
import { ApexSession, createApexMcpServer, parseProgram } from '../src/index.js';
...
test('exposes the session-oriented MCP surface and performs a one-call run', async () => {
  const app = await startFixtureApp(); const session = new ApexSession(); const server = createApexMcpServer(session); const client = new Client({ name: 'test', version: '1' });
  const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  try {
    await server.connect(serverTransport); await client.connect(clientTransport);
    expect((await client.listTools()).tools.map(tool => tool.name).sort()).toEqual(['apex_evidence', 'apex_navigate', 'apex_repair', 'apex_run', 'apex_search', 'apex_snapshot']);
    ...
    const result = await client.callTool({ name: 'apex_run', arguments: { programJson: JSON.stringify(program) } });
...
test('uses bounded MCP discovery, repair, and evidence tools', async () => {
  ...
  try {
    await server.connect(serverTransport); await client.connect(clientTransport);
    const navigated = await client.callTool({ name: 'apex_navigate', arguments: { url: app.url('/sent') } });
    expect(navigated.content[0]).toMatchObject({ type: 'text', text: expect.stringContaining('"untrusted":true') });
    const search = await client.callTool({ name: 'apex_search', arguments: { query: 'sent' } });
    ...
    const attempted = await client.callTool({ name: 'apex_run', arguments: { programJson: JSON.stringify(program) } });
    const pending = JSON.parse((attempted.content[0] as { text: string }).text);
    expect(pending).toMatchObject({ status: 'needs_repair' });
    const repaired = await client.callTool({ name: 'apex_repair', arguments: { runId: pending.repair.runId, target: { role: 'button', name: 'Sent' } } });
    const complete = JSON.parse((repaired.content[0] as { text: string }).text);
    expect(complete).toMatchObject({ status: 'success' });
    const evidence = await client.callTool({ name: 'apex_evidence', arguments: { evidenceId: complete.evidenceIds[0] } });
