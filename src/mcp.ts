...
export function createApexBrowseMcpServer(session = new ApexBrowseSession()): McpServer {
  const server = new McpServer({ name: 'apex-browse', version: '0.1.0' });

  server.registerTool('apex_browse_navigate', {
    title: 'Navigate an Apex Browse session',
    description: 'Open a URL and return one bounded semantic snapshot for discovery. For a task whose controls are already described, prefer one apex_browse_run program containing navigation and all actions.',
    inputSchema: { url: z.string().url() },
  }, async ({ url }) => { try { return text(await session.navigate(url)); } catch (cause) { return failure(cause); } });

  server.registerTool('apex_browse_run', {
    ...
    description: [
      ...
      'Targets may add aliases:["known alternate"] or scope:{role,name}. Prefer one apex_browse_run call including navigation and final expectation.',
      'A successful call returns compact receipts. If it returns needs_repair, choose only a supplied candidate and call apex_browse_repair. Arbitrary page code is never evaluated.',
    ].join(' '),
  ...
  server.registerTool('apex_browse_snapshot', {
  ...
  server.registerTool('apex_browse_search', {
  ...
  server.registerTool('apex_browse_repair', {
  ...
  server.registerTool('apex_browse_evidence', {
